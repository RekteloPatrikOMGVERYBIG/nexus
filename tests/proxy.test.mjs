...
function loadProxy(user, cookieUpdates = []) {
  let calls = 0;
  const exports = {};
  const source = ts.transpileModule(readFileSync(resolve("proxy.ts"), "utf8"), {
  ...
  vm.runInNewContext(source, {
    ...
    process: { env: {} },
    require(name) {
      if (name !== "@supabase/ssr") return localRequire(name);
      return { createServerClient(_url, _key, { cookies }) {
        return { auth: { async getUser() {
          calls++;
          for (const batch of cookieUpdates) cookies.setAll(batch, {
            "Cache-Control": "private, no-store", "Pragma": "no-cache", "Expires": "0",
          });
          return { data: { user }, error: user ? null : { message: "No valid session" } };
        } } };
      } };
    },
  });
...
for (const [name, user, path, destination] of [
  ...
  ["logged in -> nexus", { id: "test-user" }, "/nexus", null],
  ["logged in -> login", { id: "test-user" }, "/nexus/login", "/nexus"],
  ["expired/invalid session -> nexus", null, "/nexus?view=recent", "/nexus/login"],
  ["logged out -> child", null, "/nexus/private/item", "/nexus/login"],
  ["login trailing slash", null, "/nexus/login/", null],
  ["login child remains protected", null, "/nexus/login/private", "/nexus/login"],
]) {
  test(name, async () => {
    ...
    if (location) {
      assert.notEqual(location, "https://nexus.test" + path, "must not redirect to itself");
      const next = await proxy(new NextRequest(location));
      assert.equal(next.headers.get("location"), null, "redirect chain must terminate");
    }
  });
}

for (const path of ["signup", "confirm", "callback", "forgot-password", "reset-password"]) {
  test(`public auth route: ${path}`, async () => {
    const { proxy } = loadProxy(null);
    const response = await proxy(new NextRequest(`https://nexus.test/nexus/${path}`));
    assert.equal(response.headers.get("location"), null);
  });
}

for (const [user, path] of [[null, "/nexus"], [{ id: "test-user" }, "/nexus/login"], [{ id: "test-user" }, "/nexus"]]) {
  test(`SSR cookies and cache headers survive: ${Boolean(user)} ${path}`, async () => {
    const { proxy } = loadProxy(user, [
      [{ name: "session.0", value: "fixture", options: { path: "/", secure: true, sameSite: "lax", httpOnly: true } }],
...
test("matcher selects only relevant nexus pages", () => {
  ...
  for (const path of ["/nexus", "/nexus/login", "/nexus/private/item"]) {
    assert.equal(unstable_doesProxyMatch({ config, nextConfig: {}, url: path }), true, path);
  }
  for (const path of ["/", "/api/test", "/nexus-other", "/_next/static/test.js", "/_next/image?url=x", "/favicon.ico", "/logo.svg", "/nexus/_next/static/test.js", "/nexus/logo.svg"]) {
    assert.equal(unstable_doesProxyMatch({ config, nextConfig: {}, url: path }), false, path);
  }
});
